...
lenfreq_rb_mp <- ggplot(flood_sample_data_chart, aes(x=length_mm))+
  ...
  theme_bw()+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), strip.text.x = element_text(size=12), legend.position = "bottom")+
  facet_wrap(~scientific_name,nrow=2, labeller=labeller(scientific_name=scientific_names), scales= "free")
#annotate(geom="text", x=10, y=12, label="(a)",size=4)
